...
class GeometryOperation(offsb.treedi.tree.TreeOperation, ABC):
    ...
    def _generate_apply_kwargs(self, i, target, kwargs=None):
        ...
        if any([x == 0 for x in map_idx.values()]) and any(
            [x != 0 for x in map_idx.values()]
        ):
...
class AngleOperation(GeometryOperation):
    ...
    def measure(mol, idx):
        ...
        mags = np.linalg.norm(
            atoms[:, [idx[0], idx[2]], :] - atoms[:, idx[1], :][:, np.newaxis, :],
            axis=2,
        )
...
class TorsionOperation(GeometryOperation):
    ...
    def measure(mol, idx):
        ...
        costheta[mask] = (w1[mask] * w2[mask]).sum(axis=1) / (
            w1_mag[mask] * w2_mag[mask]
        )
        np.clip(costheta, -1.0, 1.0, out=costheta)
